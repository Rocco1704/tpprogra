...
public class Interfaz extends JFrame {
	...
	private void agregarLocalidad() {//TIENE VERIFICACIONE PARA ERRORES ORTOGRAFICOS
		//EJ: JCP Y JcP O BUENOS AIRES Y buenosaires
	    try {
	        ...
	        if (nombre.isEmpty() || provincia.isEmpty()) {
	            JOptionPane.showMessageDialog(this, "Nombre y provincia no pueden estar vacíos.", "Error",
	                    JOptionPane.ERROR_MESSAGE);
	            return;
	        }

	        // --- VALIDACIÓN DE DUPLICADOS ---
	        // Normalizamos la entrada actual solo para comparar
	        String entradaNormalizada = (nombre + provincia).toLowerCase().replace(" ", "");
	        
	        for (int i = 0; i < modeloTabla.getRowCount(); i++) {
	            String nombreTabla = modeloTabla.getValueAt(i, 0).toString();
	            String provinciaTabla = modeloTabla.getValueAt(i, 1).toString();
	            
	            // Normalizamos lo que ya hay en la tabla
	            String existenteNormalizado = (nombreTabla + provinciaTabla).toLowerCase().replace(" ", "");
	            
	            if (entradaNormalizada.equals(existenteNormalizado)) {
	                JOptionPane.showMessageDialog(this, "Esta localidad ya existe en la tabla.", "Advertencia",
	                        JOptionPane.WARNING_MESSAGE);
	                return; // Cortamos la ejecución para que no se agregue
	            }
	        }
	        // --------------------------------
	        ...
	        // Si pasó la validación, creamos y agregamos
	        ...
	        JOptionPane.showMessageDialog(this, "Latitud y longitud deben ser números.", "Error",
	                JOptionPane.ERROR_MESSAGE);
...
*/
	...
	private void eliminarLocalidad() {
		...
		if (fila == -1) {
			JOptionPane.showMessageDialog(this, "Seleccioná una localidad de la tabla.", "Aviso",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
	...
	private void planificar() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Los parámetros deben ser números.", "Error",
					JOptionPane.ERROR_MESSAGE);
		} catch (IllegalArgumentException ex) {
			JOptionPane.showMessageDialog(this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		}
